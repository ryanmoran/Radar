...
class JobSpec: QuickSpec {
  override func spec() {
    ...
    beforeEach {
      ...
      job = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: nil, nextBuild: nil, pipeline: pipeline)
    }
    ...
    describe("status/transientStatus") {
      ...
      context("when there is a next, and finished build") {
        beforeEach {
          job = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: finishedBuild, nextBuild: nextBuild, pipeline: pipeline)
        }
      ...
      context("when there is no next build, but there is a finished build") {
        beforeEach {
          job = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: finishedBuild, nextBuild: nil, pipeline: pipeline)
        }
      ...
      context("when the job is paused") {
        beforeEach {
          job = JobFactory.newJob(id: 1, name: "some-job", paused: true, finishedBuild: finishedBuild, nextBuild: nextBuild, pipeline: pipeline)
        }
        ...
        context("when there is no next build") {
          beforeEach {
            job = JobFactory.newJob(id: 1, name: "some-job", paused: true, finishedBuild: finishedBuild, nextBuild: nil, pipeline: pipeline)
          }
    ...
    describe("build") {
      ...
      context("when there is a finished build") {
        it("returns the finished build") {
          ...
          job = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: build, nextBuild: nil, pipeline: pipeline)
      ...
      context("when there is a next build") {
        it("returns the next build") {
          ...
          job = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: nil, nextBuild: build, pipeline: pipeline)
    ...
    describe("Equatable") {
      it("can be equated") {
        ...
        let firstJob = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: build, nextBuild: nil, pipeline: pipeline)
        let secondJob = JobFactory.newJob(id: 1, name: "some-job", paused: false, finishedBuild: build, nextBuild: nil, pipeline: pipeline)
